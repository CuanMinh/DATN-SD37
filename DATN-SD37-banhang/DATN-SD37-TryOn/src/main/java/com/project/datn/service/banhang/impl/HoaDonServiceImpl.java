...
public class HoaDonServiceImpl implements IHoaDonService {
    ...
    public void thanhToan(Long idHoaDon, HoaDonRequest hoaDonRequest) {
        ...
        Page<HoaDonChiTiet> hoaDonChiTiets = hoaDonChiTietRepository.findByHoaDonIds(idHoaDon,pageableGioHang);
...
//                maGiamGia.setNgayCapNhap(new Date());
    ...
    @Override
    public Page<HoaDon> getHoaDonByTrangThai(Integer trangThai, Pageable pageable) {
        ...
        if (authentication != null
            ...
            // Truy vấn danh sách hóa đơn theo trạng thái (kiểu `int`)
            return hoaDonRepository.findAllByTrangThaiAndTaiKhoan_Id(trangThai, taiKhoan.getId(), pageable);
        }
    ...
    @Override
    public Page<HoaDon> getHoaDonByAccount(Pageable pageable) {
        // Lấy thông tin đăng nhập từ SecurityContext
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        ...
        if (authentication != null
            ...
            // Truy vấn danh sách hóa đơn theo trạng thái (kiểu `int`)
            return hoaDonRepository.findByTaiKhoan_Id(taiKhoan.getId(), pageable);
        }
    ...
    public void cancelOrder(String maHoaDon) {
        try {
            HoaDon bill = hoaDonRepository.findByHoaDonMaHoaDon(maHoaDon)
                    .orElseThrow(() -> new NotFoundException("Không tìm thấy đơn hàng " + maHoaDon));

            if (bill.getTrangThai() != 2 && bill.getTrangThai() != 3) {
                throw new IllegalStateException("Chỉ có thể hủy đơn hàng khi trạng thái là 'Chờ xác nhận' hoặc 'Chờ lấy hàng'");
            }

            for (HoaDonChiTiet billDetail : bill.getBillDetail()) {
                ChiTietSanPham productDetail = chiTietSanPhamRepository.findById(billDetail.getChiTietSanPham().getId())
                        .orElseThrow(() -> new NotFoundException("Không tìm thấy sản phẩm " + billDetail.getChiTietSanPham().getId()));

                productDetail.setSoLuong(productDetail.getSoLuong() + billDetail.getSoLuong());
    ...
    private boolean isStatusTransitionValid(int currentStatus, int targetStatus) {
        // Ví dụ: chỉ cho phép hủy đơn từ trạng thái 2 (Chờ xác nhận) hoặc 3 (Chờ lấy hàng)
        if (targetStatus == 0 && (currentStatus == 2 || currentStatus == 3)) {
            return true;
        }
